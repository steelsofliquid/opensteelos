...
// I want to say that modifying this to comply with the style guide was surely fun
// and totally not a forme of self-imposed psychological torture >_<
...
int32_t strcmp(const char* string1, const char* string2)
{
    ...
    while (a[i] != '\0' || b[i] != '\0')
    {
        if (a[i] < b[i]) return -1;  // i may have copied some of this from libmem.cpp's memcmp, but the base structure should work anyways...
...
int8_t* strrchr(const char* string, int32_t character)
{
    ...
    */
    ...
    while (string[len]) len++; // Prerequisite copy of strlen.
