...
void* memcpy(void* destination, const void* source, size_t number)
{
    ...
    for (size_t i = 0; i < number; i++) dest[i] = sourc[i];
...
void* memmove(void* destination, const void* source, size_t number)
{
    ...
    if (dest < sourc)
    {
        for (size_t i = 0; i < number; i++) dest[i] = sourc[i]; // accidentally put i = number here
    }
    else
    {
        for (size_t i = number; i != 0; i--) dest[i - 1] = sourc[i - 1];
    }
...
void* memset(void* destination, int32_t value, size_t number)
{
    ...
    for (size_t i = 0; i < number; i++) buffer[i] = (uint8_t) value;
